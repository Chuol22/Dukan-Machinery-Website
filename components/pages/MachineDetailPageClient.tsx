...
interface MachineDetailPageClientProps {
  machine: Machine
}

export default function MachineDetailPageClient({ machine }: MachineDetailPageClientProps) {
  ...
  const tabs = [
    ...
    { id: 'features', label: 'Features', icon: <span className="text-sm">⭐</span> },
    { id: 'maintenance', label: 'Maintenance', icon: <span className="text-sm">🔧</span> },
    { id: 'warranty', label: 'Warranty', icon: <span className="text-sm">🛡️</span> },
  ]
    ...
    <div className="min-h-screen bg-white dark:bg-neutral-800">
            ...
            <div className="w-32 h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 rounded-2xl bg-white p-3 shadow-2xl transform hover:scale-105 transition-all duration-500 hover:rotate-2">
              <div className="relative w-full h-full rounded-xl overflow-hidden">
                {machine.image.endsWith('.mp4') ? (
                  <video src={machine.image} className="w-full h-full object-contain" muted loop playsInline controls />
                ) : (
                  <Image src={machine.image} alt={machine.name} fill className="object-contain" />
                )}
              </div>
            </div>
            <div className="flex-1 text-center sm:text-left">
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-white mb-2 drop-shadow-lg">{machine.name}</h1>
              <div className="inline-block bg-orange-500 px-4 py-2 rounded-full">
                <p className="text-sm font-black text-white">{machine.type} Equipment</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'process' && (
            <motion.div key="process" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              ...
              <div className="bg-neutral-50 dark:bg-neutral-700/50 rounded-xl p-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {(machine.applications || [
                    ...
                    <span key={index} className="bg-orange-500/10 text-orange-600 dark:text-orange-400 text-xs font-black px-3 py-1 rounded-full">{app}</span>
                  ))}
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'maintenance' && (
            <motion.div key="maintenance" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="bg-neutral-50 dark:bg-neutral-700/50 rounded-xl p-6">
              <h3 className="text-lg sm:text-xl font-black text-green-700 dark:text-white mb-4 flex items-center gap-2">
                <span className="text-xl">🔧</span>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  <h4 className="font-black text-green-700 dark:text-white mb-2">Daily Checks</h4>
                ...
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  <h4 className="font-black text-green-700 dark:text-white mb-2">Weekly Maintenance</h4>
                  <ul className="list-disc list-inside space-y-1 text-sm text-neutral-600 dark:text-neutral-300">
                ...
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  <h4 className="font-black text-green-700 dark:text-white mb-2">Monthly Service</h4>
                  <ul className="list-disc list-inside space-y-1 text-sm text-neutral-600 dark:text-neutral-300">
          ...
          {activeTab === 'warranty' && (
            <motion.div key="warranty" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="bg-neutral-50 dark:bg-neutral-700/50 rounded-xl p-6">
              <h3 className="text-lg sm:text-xl font-black text-green-700 dark:text-white mb-4 flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  ...
                  <p className="text-sm text-neutral-600 dark:text-neutral-300">{machine.warranty || '18 months'} comprehensive warranty covering manufacturing defects and material quality.</p>
                </div>
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  <h4 className="font-black text-green-700 dark:text-white mb-2 flex items-center gap-2">
                    <span>🎧</span>
                  ...
                  <p className="text-sm text-neutral-600 dark:text-neutral-300">24/7 phone and email support. On-site service available within 48 hours.</p>
                </div>
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  <h4 className="font-black text-green-700 dark:text-white mb-2 flex items-center gap-2">
                    <span>🚚</span>
                  ...
                  <p className="text-sm text-neutral-600 dark:text-neutral-300">Genuine spare parts available with express shipping worldwide.</p>
                </div>
                <div className="p-4 bg-white dark:bg-neutral-800 rounded-lg">
                  <h4 className="font-black text-green-700 dark:text-white mb-2 flex items-center gap-2">
                    <span>⏰</span>
                  ...
                  <p className="text-sm text-neutral-600 dark:text-neutral-300">Free technical consultation and training for the lifetime of the machine.</p>
                </div>
              </div>
            </motion.div>
          )}
        ...
        <div className="bg-neutral-100 dark:bg-neutral-700 rounded-2xl p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 max-w-3xl mx-auto">
            ...
            <div className="flex gap-3">
              <Link href={`/${locale}/order?machine=${machine.id}`}>
