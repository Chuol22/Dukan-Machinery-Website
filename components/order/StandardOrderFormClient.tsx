...
const parsePriceToNumber = (price: string): number => {
  if (price === 'Request Price') return 0
...
// Helper to format display price
const formatDisplayPrice = (price: string): string => {
  if (price === 'Request Price') return price
  return price
}

export default function StandardOrderFormClient({ onSubmit, preselectedMachineId }: StandardOrderFormProps) {
  const { machines: machinesData, loading: machinesLoading } = useMachinesData()
  ...
  const selectedMachine = machinesData.find(m => String(m.id) === formData.machineId)
  const unitPriceNumber = selectedMachine ? parsePriceToNumber(selectedMachine.price) : 0
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      setIsSubmitted(true)
      onSubmit?.(submissionData as StandardOrderData & { orderId: number; status: string })
    ...
    <div className="bg-neutral-50 dark:bg-neutral-900 rounded-2xl p-8 shadow-xl">
      ...
      <div className="space-y-5">
        <AnimatePresence mode="wait">
          ...
          {currentStep === 1 && (
            ...
            >
              ...
              <div>
                <label htmlFor="machine-select" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
                  Machine *
                </label>
                <select
                  id="machine-select"
              ...
              {selectedMachine && (
                ...
                >
                  <div className="flex gap-4">
                    <div className="relative w-24 h-24 rounded-lg overflow-hidden bg-neutral-200 dark:bg-neutral-600">
                      <MediaPlayer
                        src={selectedMachine.gallery?.[0] ?? selectedMachine.image}
                    ...
                    <div className="text-right">
                      <p className="text-lg font-bold text-primary">
                        {formatDisplayPrice(selectedMachine.price)}
                      </p>
                    </div>
                  </div>
                </motion.div>
              ...
              {selectedMachine && unitPriceNumber > 0 && (
                <div className="bg-primary/5 rounded-lg p-4">
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span>${unitPriceNumber.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Quantity:</span>
              ...
              {selectedMachine && unitPriceNumber === 0 && (
                <div className="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4 border border-yellow-200 dark:border-yellow-800">
                  <p className="text-sm text-yellow-800 dark:text-yellow-200">
                    Price available on request. Our sales team will contact you with pricing information.
                  </p>
                </div>
              )}
            </motion.div>
            ...
            >
              ...
              <div className="bg-neutral-50 dark:bg-neutral-700 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span>Unit Price:</span>
                    <span>{selectedMachine ? formatDisplayPrice(selectedMachine.price) : 'Price on request'}</span>
                  </div>
                  {unitPriceNumber > 0 && (
                  ...
                  {unitPriceNumber === 0 && selectedMachine && (
                    <div className="flex justify-between pt-2 border-t border-neutral-300 dark:border-neutral-600">
        ...
        <div className="flex justify-between mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
          <button
            onClick={handleBack}
            className={`px-6 py-2 border border-orange-600 dark:border-orange-800 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors ${
