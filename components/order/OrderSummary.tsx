...
export default function OrderSummary({ orderData, type }: OrderSummaryProps) {
  ...
  const handleEmail = () => {
    ...
    const buildBody = () => {
      ...
      const machineLine = type === 'standard'
      ...
      lines.push(`Company: ${standard?.companyName || standard?.company || '-'}`)
      lines.push(`Email: ${standard?.email || '-'}`)
      lines.push(`Phone: ${standard?.phone || '-'}`)

      lines.push('', '--- Delivery / Requirements ---')
      lines.push(`Preferred Date: ${orderData?.deliveryInfo?.preferredDate ?? '-'}`)
      lines.push(`Delivery Address: ${orderData?.deliveryInfo?.deliveryAddress ?? '-'}`)
      lines.push(`Special Instructions: ${orderData?.deliveryInfo?.specialInstructions ?? '-'}`)
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden">
      ...
      <div className="p-6 md:p-8">
        <div className="flex justify-end gap-3 mb-6 print:hidden">
          ...
          <button onClick={handleEmail} className="p-2 text-gray-600 hover:text-primary" title="Email order">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
              <p>
                <span className="font-medium">Name:</span> {orderData.customerInfo?.fullName || orderData.contactInfo?.name}
              </p>
              <p className="mt-1">
                <span className="font-medium">Company:</span> {orderData.customerInfo?.companyName || orderData.contactInfo?.company}
              </p>
              <p className="mt-1">
                <span className="font-medium">Email:</span> {orderData.customerInfo?.email || orderData.contactInfo?.email}
              </p>
              <p className="mt-1">
                <span className="font-medium">Phone:</span> {orderData.customerInfo?.phone || orderData.contactInfo?.phone}
              </p>
            </div>
          </div>
              ...
              {type === 'standard' ? (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between pt-2 border-t">
                    ...
                    <span className="font-bold text-primary text-lg">ETB {orderData.totalPrice?.toLocaleString()}</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>
                      {orderData.requiredCapacity?.value} {orderData.requiredCapacity?.unit}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-medium capitalize">Power Source:</span>
          ...
          <div className="border-t pt-6">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <p>
                  Our sales team will contact you within <strong className="text-primary">24 hours</strong> to confirm your order.
                </p>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center">
                ...
                <p>You will receive a proforma invoice and payment instructions via email.</p>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center">
                  <span className="text-primary text-sm font-bold">3</span>
                </div>
                <p>Production begins after payment confirmation. Lead time: 2-3 weeks.</p>
              </div>
            </div>
          </div>
