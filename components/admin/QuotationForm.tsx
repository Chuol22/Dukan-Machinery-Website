    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          body?.error ||
            body?.message ||
            `Quotation failed (HTTP ${res.status})`,
        );
      }
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <label className="block">
          ...
          <input
            className="mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-2"
            type="number"
            min={0}
            step={0.01}
            value={machineCost}
      ...
      <div className="flex gap-3">
        <button
          type="button"
          onClick={onClose}
          className="flex-1 py-3 border border-gray-200 dark:border-gray-800 rounded-xl font-bold text-gray-700 dark:text-gray-300"
        >
          Cancel
        </button>
        <button
