...
export async function PUT(
  ...
  try {
    ...
    const adminNotes =
      body?.admin_notes !== undefined ? String(body.admin_notes) : undefined;
    const assignedTo =
      body?.assigned_to !== undefined ? String(body.assigned_to) : undefined;
    ...
    const mergedNotes = [existing.admin_notes, adminNotes]
      .filter(Boolean)
      .join("\n\n");
    const nextAssigned =
      assignedTo ??
...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      `Deleted ${deletedNotifications.count} notifications for order ${id}`,
    );
