...
export default function MachinesClient({
  ...
  const handleDeleteMachine = async (machine: Machine) => {
    if (
      !confirm(
        `Delete "${machine.name}" from the catalog? This cannot be undone.`,
      )
    )
      return;
    ...
    try {
      ...
      showToast(
        `${machine.name} has been removed from the catalog.`,
        "success",
      );
    } catch (e) {
  ...
  const handleSaveMachine = async () => {
    ...
    try {
      if (editingMachine) {
        ...
        setRows((prev) =>
          prev.map((m) =>
            m.id === editingMachine.id ? ({ ...data.machine } as Machine) : m,
          ),
        );
    ...
    try {
      ...
      const res = await fetch(`/api/machines/${machine.id}`, {
        ...
        body: JSON.stringify({
          ...
          available: !["out_of_stock", "maintenance", "discontinued"].includes(
            nextStatus,
          ),
        }),
      });
      ...
      showToast(
        `${machine.name} status updated to ${nextStatus.replace(/_/g, " ")}.`,
        "success",
      );
    } catch (e) {
      const msg =
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-gray-150 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
                    ...
                    <th className="py-3.5 px-4 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs hidden lg:table-cell">
                      Category
                    </th>
                    <th className="py-3.5 px-4 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs hidden md:table-cell">
                      Capacity
                    </th>
                    <th className="py-3.5 px-4 font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-xs hidden md:table-cell">
                      Power
                      ...
                      <td className="py-4 px-4">
                        <select
                          value={
                            ...
                            (machine.available ? "available" : "out_of_stock")
                          }
                          onChange={(event) =>
                            updateAvailability(machine, event.target.value)
                          }
            ...
            <div className="p-6 space-y-5 flex-1 overflow-y-auto">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <label
                      ...
                      className={`inline-flex items-center space-x-2 px-4 py-2.5 rounded-xl font-semibold text-sm cursor-pointer transition-colors ${
                        uploadingImage
                  ...
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Upload images to Cloudinary or enter a Cloudinary URL. Max
                    file size: 5MB.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
                    placeholder="e.g. 500-1000 kg/hr"
                  />
                  {formErrors.capacity && (
                    <p className="mt-1 text-xs text-red-500">
                      {formErrors.capacity}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500"
                  placeholder="e.g. ETB 250,000 or Call for price"
                />
                {formErrors.price && (
                  <p className="mt-1 text-xs text-red-500">
                    {formErrors.price}
