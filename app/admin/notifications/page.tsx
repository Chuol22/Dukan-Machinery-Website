        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-150 dark:divide-gray-800">
            {filtered.map((notif) => (
              <div
                key={notif.id}
                className={`flex items-start space-x-4 p-5 hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors border-l-4 ${getBorderColor(notif.type)} ${!notif.read ? 'bg-orange-50/20 dark:bg-orange-950/5' : ''
                  }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className={`text-sm font-bold ${!notif.read ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'
                        }`}>
                        {notif.title}
                      </p>
...
function StatCard({
  ...
  return (
    <div className={`${bg} rounded-xl p-4 border border-gray-150 dark:border-gray-800`}>
      <div className="flex items-center space-x-3">
