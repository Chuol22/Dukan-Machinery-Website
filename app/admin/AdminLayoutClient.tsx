...
export default function AdminLayoutClient({
  ...
  const isActive = (href: string) => {
    ...
    if (href === "/admin/quotations")
      return pathname.startsWith("/admin/quotations");
    ...
    if (href === "/admin/machines/new")
      return pathname === "/admin/machines/new";
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col md:flex-row">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-48 bg-gray-300 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col justify-between transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:h-screen shrink-0 ${
      ...
      >
        <div className="flex flex-col flex-1 overflow-y-auto">
          ...
          <nav className="p-4 space-y-1 flex-1">
            ...
            <NavLink
              item={navItems[5]}
              isActive={isActive(navItems[5].href)}
              onClick={() => setSidebarOpen(false)}
            />
            <NavLink
              item={navItems[6]}
              isActive={isActive(navItems[6].href)}
              onClick={() => setSidebarOpen(false)}
            />
            <NavLink
              item={navItems[7]}
              isActive={isActive(navItems[7].href)}
              onClick={() => setSidebarOpen(false)}
            />
            ...
            <NavLink
              item={navItems[8]}
              isActive={isActive(navItems[8].href)}
              onClick={() => setSidebarOpen(false)}
            />
      ...
      <div className="flex-1 flex flex-col md:h-screen md:overflow-y-auto min-w-0">
        ...
        <footer className="w-full py-4 px-6 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 text-center text-xs text-gray-500 dark:text-gray-400 mt-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p>
            © {new Date().getFullYear()} Dukan Machinery. All rights reserved.
          </p>
          <span className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded font-mono text-[10px]">
            Access:{" "}
            {user?.role === "SUPER_ADMIN" ? "Full Control" : "View & Update"}
          </span>
        </footer>
      </div>
    </div>
